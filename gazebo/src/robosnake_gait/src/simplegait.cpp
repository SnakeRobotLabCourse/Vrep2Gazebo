...
std::vector<double> SimpleGait::getAngle(double dt, int numberJoints)
{
	t += dt;
	if (t > 20.0)
	{
	...
	for(unsigned i = 0; i < numberJoints; i++)
	{
		...
		if (i % 2 == 1)
		{
			result.push_back((a_v[0] * (1.0 - s) + a_v[1] * s) * sin(t * (speed[0] * (1.0 - s) + speed[1] * s) 
				+ (i / 2 + 1)  * (p_v[0] * (1.0 - s) + p_v[1] * s)));
		} else 
		{
			result.push_back((a_h[0] * (1.0 - s) + a_h[1] * s) * cos(t * (speed[0] * (1.0 - s) + speed[1] * s) 
				+ (i / 2 + 1) * (p_h[0] * (1.0 - s) + p_h[1] * s)));
		}
	}
	return result;	
}
